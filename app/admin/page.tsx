...
export default function AdminPage() {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem("hexagonal_admin_key");
    if (saved) {
      setKey(saved);
    }
  }, []);
  ...
  useEffect(() => {
    if (key) {
      sessionStorage.setItem("hexagonal_admin_key", key);
      void loadWaitlist();
    }
  }, [key]);
    ...
    <main className="min-h-screen bg-[#0B0F14] text-white p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold">HEXAGONAL Admin</h1>
            <p className="text-sm text-zinc-400 mt-1">Manage waitlist access</p>
          </div>
              ...
              {users.map((user) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-zinc-500 mt-1">{user.created_at}</p>
                  </div>
                    ...
                    {user.status === "pending" && (
                      <>
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-green-500 text-black text-xs font-semibold disabled:opacity-50"
                        >
                          {actingId === user.id ? "..." : "Approve"}
                        </button>
                        <button
